...
from keras.layers import Input, Embedding, LSTM, TimeDistributed, Dense, SimpleRNN, Activation, dot, concatenate, Bidirectional, GRU, Lambda
...
def encode_sequences(encoding_dict, sequences, max_length):
    ...
    """
    encoded_data = np.zeros(shape=(len(list(sequences)), max_length))
    for i in range(len(list(sequences))):
...
def build_model_local(params_path = 'test/params', enc_lstm_units = 128, unroll = True, use_gru=False, optimizer='adam', display_summary=True):
    """
    Build keras model
    ...
    window_width = 2
    context_states=[]
    for state in range(max_input_length):
        aligned_position = state
        left = int(aligned_position - window_width
                           if aligned_position - window_width >= 0
                           else 0)
        right = int(left + (2*window_width)-1
                            if left+ (2*window_width)-1 < max_input_length
                            else max_input_length-1)
        if (right-left+1< 2*window_width):
            left=right-(2*window_width)+1
        encoder_outs_window = Lambda(lambda x: x[:, left:right, :])(encoder_outs)
        decoder_state=Lambda(lambda x: x[:, state, :])(decoder)
        
        attention_for_window = dot([decoder_state, encoder_outs_window], axes=[1, 2])
        attention_for_window = Activation('softmax')(attention_for_window)
        
        context_state_for_window = dot([attention_for_window, encoder_outs_window], axes=[1,1])
        context_states.append(context_state_for_window)

	
    # luong attention
    context = Lambda(lambda x: permute_dimensions(tf.stack(x), pattern=(1, 0, 2)))(context_states)
...
if __name__ == '__main__':
    ...
    with open('SyllEnglishtraining.txt','r') as r1,open('SyllHinditraining.txt','r') as r2:
        #input_data=csv.reader(r1)
        #output_data=csv.reader(r2)
        input_data=r1.readlines()
        output_data=r2.readlines()

        input_list=[]
        output_list=[]
        for word1,word2 in zip(input_data,output_data):
            input_list.append(word1.rstrip('\n'))
            output_list.append(word2.rstrip('\n'))

        build_params(input_data = input_list, output_data = output_list, params_path = 'test/paramsSyllv2', max_lenghts=(30, 30))

        model, params = build_model(params_path='test/paramsSyllv2', use_gru=True)

        input_data, output_data = convert_training_data(input_list, output_list, params)
        #input_val, output_val = convert_training_data(input_val, output_val, params)

        checkpoint = ModelCheckpoint('test/checkpointSyllv2', monitor='accuracy', verbose=1, save_best_only=True, mode='max')
        callbacks_list = [checkpoint]
        hist=model.fit(input_data, output_data, batch_size=200, epochs=25, callbacks=callbacks_list)
